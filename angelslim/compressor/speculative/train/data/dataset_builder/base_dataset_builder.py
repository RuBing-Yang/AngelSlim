...
class DatasetBuilder(metaclass=ABCMeta):
    ...
    def build_dataset(
        self, datapath: str, num_proc: int = 8, sample_num: int = 100000
    ) -> Dataset:
        try:
            ...
            # Sample a subset if sample_num is specified
            if sample_num is not None and 0 < sample_num < len(ds):
                ds = ds.select(range(min(sample_num, len(ds))))
            ...
            for key in processed_ds.column_names:
                print(f"Column '{key}' length: {len(processed_ds[key])}")
            ...
            print(f"dataset keys: {processed_ds.column_names}")
